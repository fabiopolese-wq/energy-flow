...
const PlanCardExpanded: React.FC<Props> = ({ onClose }) => {
  ...
  return (
    <div className="rounded-2xl border border-[#6E40FF] p-6 bg-white">
      <div className="grid items-start gap-6 grid-cols-1 lg:grid-cols-[1fr_280px]">
        {/* LEFT: CONTENT (must span the 1fr column) */}
        <section className="min-w-0 w-full lg:col-start-1 lg:col-end-2">
          {/* header rows, rates toggle, badges, divider, compare, bio */}
          ...
          <button
            ...
            className="mt-2 inline-flex items-center gap-1 text-[14px] text-[#111827] hover:underline"
          >
            View unit rates and standing charges
            <span className={`transition-transform ${openRates ? 'rotate-180' : ''}`}>▴</span>
          </button>

          {/* rates toggle block */}
          {openRates && (
            <div id={listId} className="mt-2 w-full space-y-2">
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6 w-full">
              ...
              <div className="min-w-0 w-full">
                ...
                <div className="mt-3 space-y-2.5 w-full">
                  <div className="flex items-center justify-between w-full">
                    <div className="text-[12px] text-[#6B7280]">Pricing type</div>
                    <div className="text-[13px] font-semibold text-[#111827]">Variable (changes with cap)</div>
                  </div>
                  <div className="flex items-center justify-between w-full">
                    <div className="text-[12px] text-[#6B7280]">Contract length</div>
                    <div className="text-[13px] font-semibold text-[#111827]">Open-ended</div>
                  </div>
                </div>
              </div>
              ...
              <div className="min-w-0 w-full">
                ...
                <div className="mt-3 space-y-2.5 w-full">
                  <div className="flex items-center justify-between w-full">
                    <div className="text-[12px] text-[#6B7280]">Pricing type</div>
                    <div className="text-[13px] font-semibold text-[#111827]">Fixed (locked for 12 months)</div>
                  </div>
                  <div className="flex items-center justify-between w-full">
                    <div className="text-[12px] text-[#6B7280]">Contract length</div>
                    <div className="text-[13px] font-semibold text-[#111827]">12 months</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* RIGHT: PRICE PANEL (fixed width column) */}
        <aside className="w-full lg:w-[280px] lg:col-start-2 lg:col-end-3 lg:justify-self-end order-2 lg:order-none">
      ...
      {/* footer buttons row sits outside grid so it spans full width */}
      <div className="mt-6 flex items-center justify-between">
        ...
        <button onClick={onClose} className="inline-flex items-center gap-1 text-[14px] font-semibold text-[#111827] focus:outline-none focus:ring-2 focus:ring-[#6E40FF] focus:ring-offset-2 rounded-md px-2 py-1">
